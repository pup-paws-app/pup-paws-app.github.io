...
function openDB_onupgradeneeded(evt) {
  ...
  try {
    ...
    var store = db.createObjectStore(dbGlobals.storeName, {keyPath: "imageId"});
    var imageIdIndex = store.createIndex("by_imageId", "imageId", {unique: true});
    ...
    // db.createObjectStore(dbGlobals.storeName); // Create the object store such that each object in the store will be given an "ID" property that is auto-incremented monotonically. Thus, files of the same name can be stored in the database.
  } catch (ex) {
...
function handleFileSelection(evt) {
  ...
  try {
    ...
    for (var i = 0, file; file = files[i]; i++) {
      ...
      var putRequest = objectStore.put({imageFile: file, imageId: "dog1"}); // The put() method will update an existing record, whereas the add() method won't.
...
function getImageByImageId(imageId, callback) {
  var db = dbGlobals.db;

  var tx = db.transaction(dbGlobals.storeName, "readonly");
  var store = tx.objectStore(dbGlobals.storeName);
  var index = store.index("by_imageId");

  var request = index.get(imageId);
  ...
  //     console.log("Here's the image file:");
  //     console.log(matching.imageFile);
  //     return matching.imageFile;
  //   } else {
  //     // No match was found.
...
function displayDB() {
  ...
  try {
    ...
    var index = objectStore.index("by_imageId");
